...
public class GetFixedExpenseGatewayImplTests {
    ...
    public void shouldReturnOptionalOfFixedExpenseModel() {
        User user = MocksFactory.userFactory();
        FixedExpenseJpaEntity entity = MocksFactory.fixedExpenseJpaEntityFactory(user);
        FixedExpenseModel fixedExpense = MocksFactory.fixedExpenseModelFactory(entity);

        when(this.repository.findByIdAndUserId(entity.getId(), entity.getUser().getId())).thenReturn(Optional.of(entity));
        when(this.mapper.map(entity)).thenReturn(fixedExpense);

        Optional<FixedExpenseModel> result = this.getFixedExpenseGateway.getModel(entity.getId(), entity.getUser().getId());

        assertThat(result).isPresent();
        assertThat(result.get().getCategory()).isEqualTo(fixedExpense.getCategory());
